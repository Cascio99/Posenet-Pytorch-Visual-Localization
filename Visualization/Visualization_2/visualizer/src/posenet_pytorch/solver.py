...
class Solver():
    def __init__(self, data_loader, config):
        ...
        """###"""
        self.model_save_path = '/home/ysw/catkin_ws/src/visualizer/src/posenet_pytorch/models_%s' % self.data_name
        # self.model_save_path = 'models_NCLT_cam4_2seqs_3m'
        # self.summary_save_path = 'summary_%s' % self.data_name
        self.summary_save_path = '/home/ysw/catkin_ws/src/visualizer/src/posenet_pytorch/summary_%s' % self.data_name
        """###"""
        ...
        ####################################################################################################
        self.cnt = 0
        self.data_list = list(self.data_loader)
        self.pos_loss_arr = []
        self.ori_loss_arr = []
        self.true_pose_list = []
        self.estim_pose_list = []
        print("HELP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        
        rospy.init_node('posenet_pytorch', anonymous=True)
        rospy.Subscriber('/image',Image, self.callback)
        self.pub = rospy.Publisher('pred_pose', Pose, queue_size=100) #0

    def callback(self, img_msg):
        if self.cnt >= len(self.data_list):
            rospy.logwarn(f'Index {self.cnt} is out of range')
            exit()

        inputs, poses = self.data_list[self.cnt]    # indexing possible? <- multiprocessing
        
        rospy.loginfo(f'Processing batch {self.cnt}: inputs shape: {inputs.shape}, poses shape: {poses.shape}')
        ...
        ########## publish quaternion form
        ori_out_tmp = ori_out.squeeze(0).detach().cpu().numpy()
        ori_out = quat_to_euler(ori_out.squeeze(0).detach().cpu().numpy())
        # print('pos out', pos_out)
        # print('ori_out', ori_out)
        ####################################################################################################
        # rate = rospy.Rate(100)
        # rospy.sleep(0.1)  # instead of rate.sleep() after publish
        print("Debugging... cnt: ", self.cnt, ", position: ", pos_out)

        p = Pose()
        p.position.x = pos_out[0]
        p.position.y = pos_out[1]
        p.position.z = pos_out[2]
        # Quaternion form? not euler form
        p.orientation.x = ori_out_tmp[0]
        p.orientation.y = ori_out_tmp[1]
        p.orientation.z = ori_out_tmp[2]
        p.orientation.w = ori_out_tmp[3]
        # p.orientation = ori_out
        # rate.sleep()    # necessary???
        self.cnt += 1
        ####################################################################################################
        ...
        self.true_pose_list.append(np.hstack((pos_true, ori_true)))
        if loss_pos_print < 20:
            self.estim_pose_list.append(np.hstack((pos_out, ori_out)))
        # print(pos_out)
        # print(pos_true)
        self.pos_loss_arr.append(loss_pos_print)
        self.ori_loss_arr.append(loss_ori_print)
        print('{}th Error: pos error {:.3f} / ori error {:.3f}'.format(self.cnt, loss_pos_print, loss_ori_print))
    ...
    def test(self):
        ...
        """for i, (inputs, poses) in enumerate(self.data_loader):
            ...
            print('ori_out', ori_out)
            ####################################################################################################
            pub = rospy.Publisher('/pred_pose', Pose, queue_size=100)
            rospy.init_node('posenet_node', anonymous=True)
            rate = rospy.Rate(100)
            # rospy.sleep(0.1)  # instead of rate.sleep() after publish
            def callback(img):
                print("Debugging... self.cnt: ", self.cnt)
                print(pos_out)
                p = Pose()
                p.position.x = pos_out[0]
                p.position.y = pos_out[1]
                ...
                # rate.sleep()    # necessary???
                self.cnt += 1
            img = rospy.Subscriber('/image', Image, callback)
            rospy.spin()
            # print("Debugging... self.cnt: ", self.cnt)
            # print(pos_out)
            ####################################################################################################
        ...
        position_error = np.median(self.pos_loss_arr)
        rotation_error = np.median(self.ori_loss_arr)

        print('=' * 20)
        print('Overall median pose errer {:.3f} / {:.3f}'.format(position_error, rotation_error))
        print('Overall average pose errer {:.3f} / {:.3f}'.format(np.mean(self.pos_loss_arr), np.mean(self.ori_loss_arr)))

        f_true = self.summary_save_path + '/pose_true.csv'
        f_estim = self.summary_save_path + '/pose_estim.csv'
        np.savetxt(f_true, self.true_pose_list, delimiter=',')
        np.savetxt(f_estim, self.estim_pose_list, delimiter=',')
