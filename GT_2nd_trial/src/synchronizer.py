...
def callback(img_msg, odom_msg):
    ...
    # write pose_information on txt file
    ...
    with open(pose_path, "a") as f:
        if img_pose_cnt  == 1:
            intro = "\nImageFile, Camera Position [X Y Z W P Q R]\n\n"  # position: x~z, orientation: w, p(x), q(y), r(z)
            f.write(intro)
        f.write(pose_info)
...
def listener():
    ...
    odom_sub = Subscriber("/integrated_to_init", Odometry)

    # image_pose time-synchronization
    ts = ApproximateTimeSynchronizer([img_sub,odom_sub], queue_size=1000, slop=1)
