...
class Solver():
    ...
    def test(self):
        ...
        for i, (inputs, poses) in enumerate(self.data_loader):
            ...
            pub = rospy.Publisher('pose', Pose, queue_size=10) # 토픽 이름 지정
            rospy.init_node('pose_publisher', anonymous=True) # 노드 생성
            ...
            if self.config.bayesian:
                ...
                # rospy.loginfo(pos_out, ori_out) # 터미널에 출력
                # pub.publish(pos_out, ori_out)   # 2개를 하나로 만들고 publish해야 함
                #pub.publish(ori_out)

                p = Pose()
                p.position = pos_out
                
                # p.orientation.x = ori_out[0]
                # p.orientation.y = ori_out[1]
                # p.orientation.z = ori_out[2]
