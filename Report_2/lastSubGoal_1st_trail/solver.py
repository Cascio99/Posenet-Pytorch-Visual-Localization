...
# added for visual localization in rviz
import rospy
...
class Solver():
    ...
    def test(self):
        ...
        for i, (inputs, poses) in enumerate(self.data_loader):
            ...
            """#########################################"""
            # added for marker
            train_marker = rospy.Publisher('train_marker', Marker, queue_size=10) # 토픽 이름 지정
            test_marker = rospy.Publisher('test_marker', Marker, queue_size=10) # 토픽 이름 지정
            """#########################################"""

            rospy.init_node('rviz_node', anonymous=True) # 노드 생성
            ...
            # mark GT-pose
            tr = Marker()
            tr.header.frame_id = "world"    # frame_id = "map"
            tr.type = tr.POINTS
            tr.action = tr.ADD
                
            p = Pose()
            p.position = pos_true
            p.orientation = ori_true
            tr.pose = p
            ...
            tr.color.g = 1.0
            tr.color.b = 0.0
            train_marker.publish(train_marker)  # 토픽 보내기

            # mark predict-pose
            te = Marker()
            te.header.frame_id = "world"    # frame_id = "map"
            te.type = te.POINTS
            te.action = te.ADD

            q = Pose()
            q.position = pos_out
            q.orientation = ori_out
            te.pose = q
            ...
            te.color.g = 1.0
            te.color.b = 0.0
            test_marker.publish(test_marker)  # 토픽 보내기
            """#########################################"""
